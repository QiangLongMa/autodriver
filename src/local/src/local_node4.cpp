...
class local_trajs_node : public rclcpp::Node{
    public:
        ...
        void GetLocalPath(){
            ...
            if (!run_stop_flag) {
                ...
                if(first_run){
                    ...
                    double obsmins, obswigth;
                    std::vector<obses_sd> rigthobstaclelist;//右边障碍物集合
                    CalObsMInS(obsmins,globalPath(6,index),obswigth,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal, rigthobstaclelist);
                    if (!rigthobstaclelist.empty()) {   
                        obsmins = rigthobstaclelist.front().min_s;
                        obswigth = rigthobstaclelist.front().wigth;
                    }
                    ...
                    if(!CarCloseTerminalFlag){
                        ...
                        //write();
                        //之前规划的路径没有碰撞或着没有时间超时 //路径点与障碍物有没有碰撞 只有当有碰撞时，，才进行借道超车  //没有碰撞 
                        if(!obs::HasOverlap(all_local_pointss,GlobalcoordinatesystemObsesLimitinlocal,optTrajxy) &&
                            ...
                            if(Decisionflags_.DecelerateFlag && !CarCloseTerminalFlag){ //减速过程 并且不接近终点 这是因为前面有障碍物阻挡停车的
                                ...
                                if (Decisionflags_.DriveStraightLineFlag) {
                                    ...
                                    none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
                                    if(none == 1){ //找到路径 使用此次路径 下次进行规划 直线行使状态下 
                                    ...
                                    LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                                globalPath,DpAllDistance,DpAllDistance,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                                start_l,end_l,delta_l,target_v,-1.5,Decisionflags_, 0, false, false, 0, 0);
                                    none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
                                    if (none == 1) {
                                        Decisionflags_.DecelerateFlag = false;
                                        Numbercycles = 100;
                                    ...
                                    if (obsmins == 1e16) { //右侧车道此时无障碍物
                                        ...
                                        if (FrentPoint_.d < 0){//车辆还在右侧车道 这个情况应该是先向左侧超车 但是超车之后又无法行使，右侧车道减速行使 
                                            start_l = -1.5; end_l = -2.0;
                                            delta_l = 0.5; target_v = 5;
                                            target_l = -1.5; 
                                            LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                                globalPath,DpAllDistance,DpSingleDistance,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                                start_l,end_l,delta_l, target_v, target_l, Decisionflags_, 0, false, true, 10, 2); //30 20 10 
                                            none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);                           
                                        } else { //在左侧车道 
                                            if (!closeroute) {
                                                start_l = 1.5; end_l = 1.5;
                                            ...
                                            if (none == 1){ //可以继续超车 
                                                ...
                                                start_l = 0.5; end_l = -0.5;
                                                target_l = 0.5;
                                                LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                                    globalPath,30,10,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                                    start_l,end_l,delta_l,target_v,target_l,Decisionflags_, 0, false, true, 10, 2);
                                                none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
                                            }
                                        }
                                        if (none == 0) {
                                ...
                                if (none){//找到路径再进行之后的操作 确保初始时找到局部路径
                                    ...
                                    if (car(2) <= 0.1) {
                                        ...
                                        if (wait_time_flag) {
                                            ...
                                            wait_time_flag = false;
                                        }                                       
                                        std::chrono::duration<double> duration = first_time - second_time;
                                        second_time = first_time;
                                        wait_time += duration.count();
                                        if (wait_time > 3) { //等待时间大于三秒种 
                                            start_l = 2.5; end_l = -2.5;
                                            delta_l = 0.5; target_v = 5;
                                            LOCAL_.setPatam(gpsA, car(2),FrentPoint_.s,FrentPoint_.d,dl,ddl,globalPath,DpAllDistance,DpAllDistance,index,onlylocalpath,
                                            GlobalcoordinatesystemObsesLimitinlocal,start_l,end_l,delta_l,target_v,-1.5,Decisionflags_,0,false, false, 0, 0);
                                            none=LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
                                            if (none) {
                                                optTrajsd.clear();
                                                optTrajxy.resize(lastOptTrajxy.rows(), lastOptTrajxy.cols());
                            ...
                            if(Decisionflags_.ObstacleAvoidanceFlag || Decisionflags_.righttoleftlane){
                                ...
                                LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                        ...
                                                        start_l,end_l,delta_l,target_v,target_l, Decisionflags_,0,false, false, 0, 0);
                                none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
                                double widthinrangesnumber = 0.0;
                                for (size_t i = 0; i < rigthobstaclelist.size(); ++i) { //车辆的数量
                                    if (rigthobstaclelist[i].wigth > 1.0 ) {
                                        widthinrangesnumber ++;
                                    }
                                }
                                if( rigthobstaclelist.empty() && none == 1){//右边车道中无障碍物且要计算返回右侧车道的线路不会发生碰撞  这个是解决多个障碍物联排的场景  2个以上的车辆数量
                                    //CalStartCarD(FrentPoint_.d,-1.5,start_l,end_l);
                                    ...
                                    if(none==0){ //超车路径发生碰撞  先向右侧寻路30m   向右侧车道寻路 20m远 10m 不能占用左侧车道 
                                        ...
                                        if(none==0){
                                            ...
                                            LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                            globalPath,20,10,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                            start_l,end_l,delta_l,target_v,target_l,Decisionflags_,0,false, true, 10, 1);
                                            none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);                                        
                                            if (none == 0 && FrentPoint_.d >= -0.5){ //考虑中间行使  左侧超车失败，右侧也无法通行
                                                start_l = 0.5; end_l = -0.5;
                                                delta_l = 0.5; target_v = 5;
                                                target_l = 0.0;
                                                LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                ...
                                if(obsmins == 1e16){
                                    ...
                                    LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                    globalPath,DpAllDistance,DpSingleDistance,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                    start_l,end_l,delta_l,target_v,targetl,Decisionflags_,0,false, false, 0, 0);//里面进行了碰撞检测 外面无需再进行了 
                                    none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
                                    if(none == 0){//未寻找到路径 右侧直线车道中有碰撞 直接左侧车道超车
                                        ...
                                        LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                    globalPath,DpAllDistance,DpSingleDistance,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                    start_l,end_l,delta_l,target_v,1.5,Decisionflags_,0,false, false, 0, 0);//里面进行了碰撞检测 外面无需再进行了 
                                        none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
                                        if(none==0){ //借道超车时 有碰撞
                                            target_v = 0;
                                            Decisionflags_.DecelerateFlag = true;
                                            LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                        globalPath,20,10,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                        start_l,end_l,delta_l,target_v,1.5,Decisionflags_,0,false, true, 10, 1);//里面进行了碰撞检测 外面无需再进行了 
                                            none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);                                         
                                            if (none == 0){
                                                IsUseVehicleFlag = true;
                                                size_t T = 0.5;//距离较近 规定车辆1秒钟必须停下来
                                                double deceleration = - car(2) / T;
                                                for(size_t i = 0; i < optTrajxy.cols(); ++i){
                                                    if(optTrajxy(5, i) < optTrajxy(5, closestIndex)){
                                                        continue;
                                    ...
                                    if(!closeroute){//不在路口 
                                        ...
                                        if(none == 0){//车道内超车失败 
                                            ...
                                            if(none==0){//借到超车也失败 道路全被堵死 直线行使时 减速停车  
                                                Decisionflags_.ObstacleAvoidanceFlag =false;
                                                Decisionflags_.DecelerateFlag =true;
                                                Decisionflags_.DriveStraightLineFlag=true;//
                                                Decisionflags_.righttoleftlane=false;
                                                ...
                                                double CarAndObsStopDistanceThreshold = 4;//车辆和障碍物之间停止的距离阈值
                                                ...
                                                // std::cout<<"obsmins: "<<obsmins<<std::endl;
                                                // std::cout<<" FrentPoint_.s: "<< FrentPoint_.s<<std::endl;
                                                IsUseVehicleFlag = true;
                                            ...
                                            LOCAL_.setPatam(gpsA,car(2),local_start_s,local_start_l,dl,ddl,
                                                        globalPath,DpAllDistance,DpSingleDistance,index,onlylocalpath,GlobalcoordinatesystemObsesLimitinlocal,
                                                        start_l,end_l,delta_l,target_v,-1,Decisionflags_,obsmins,false, false, 0, 0);
                                            none = LOCAL_.GetoptTrajxy(lastOptTrajxy,lastOptTrajsd);
        ...
        void CalObsMInS(double &ObsMinS,double car_s,double &obs_length,std::vector<obses_sd>& obses,std::vector<Eigen::VectorXd> &globalobs_in_local
                ,std::vector<obses_sd> &rightobstaclelist){
            ...
            for (size_t i = 0; i < obses_size; ++i){
                ...
                if ((min_s - car_s) <= Kthreshold){
                    ...
                    if(obs::decide_obs_true_false(obses[i],-0.5,-2.6) == 1){ //在这个范围内的都是算成障碍物 1.5-0.9-0.3  1.5+0.9+0.3 
                        rightobstaclelist.push_back(obses[i]);
                    }
                }
            }
            if (!rightobstaclelist.empty()){
                auto comp = [](const obses_sd& p1, const obses_sd& p2){
                    return p1.min_s < p2.min_s;
                };
                std::sort(rightobstaclelist.begin(), rightobstaclelist.end(), comp);// min_s 由小到大进行排序 
            }
        }
        ...
        /*void CalObsMInS(double &ObsMinS,double car_s,double &obs_length,std::vector<obses_sd>& obses,std::vector<Eigen::VectorXd> &globalobs_in_local
                ,std::vector<obses_sd> &rightobstaclelist){
            static std::vector<obses_sd> previous_obses;
            size_t obses_size = obses.size();
            ObsMinS = 1e16;
            double obs_s;
            double obs_l;
            size_t mindex = 1e4;
            double Kthreshold = 25; 
            if(obses_size == 0) { //没有障碍物 
                previous_obses.clear(); //右侧障碍物也为空 
                return;
            }
            //当前有障碍物
            double current_min_s = 1e16;//跟踪当前帧最小的min_s 
            // 遍历当前帧的障碍物
            for (size_t i = 0; i < obses_size; ++i){
                double min_s = obses[i].min_s;
                if ((min_s - car_s) <= Kthreshold){
                    //std::cout<<"obs "<<obs::decide_obs_true_false(obses_limit_SD[i],-0.3,-2.9)<<std::endl;
                    if(obs::decide_obs_true_false(obses[i],-0.5,-2.6) == 1){ //在这个范围内的都是算成障碍物 1.5-0.9-0.3  1.5+0.9+0.3 
                        rightobstaclelist.push_back(obses[i]);
            ...
            if (!rightobstaclelist.empty()){
                ...
                std::sort(rightobstaclelist.begin(), rightobstaclelist.end(), comp);// min_s 由小到大进行排序 
                current_min_s = std::min(current_min_s, rightobstaclelist.front().min_s);
            } else { 
                previous_obses.clear(); //右侧障碍物也为空 
                return;
            } //右侧没有障碍物 
            // 如果有上一帧的障碍物数据，则判断min_s是否变大
            if (!previous_obses.empty()) {
                // 取上一帧的最小min_s
                double previous_min_s = 1e16;
                for (const auto& prev_obs : previous_obses) {
                    double prev_min_s = prev_obs.min_s;
                    previous_min_s = std::min(previous_min_s, prev_min_s);
                }
                // 如果当前帧的最小min_s大于上一帧的最小min_s，则认为障碍物远离
                if (current_min_s > (previous_min_s + 0.2)) {
                    ObsMinS = 1e16;  // 如果障碍物在远离，重置ObsMinS为大值
                } else {
                    ObsMinS = current_min_s;  // 否则，更新ObsMinS为当前帧的最小min_s
                }
            }
            previous_obses = rightobstaclelist;
            // if(mindex == 1e4){
            //     obs_length = 0;
            // }else{
        ...
        */
        ...
        //发布是否启动减速 
        void PubDecelerateFlag(bool flag){
        ...
        std::array<double, 6> Decidestartsl(tool::frentPoint &FrentPoint,int position_matched_index,bool flag,
            ...
            if (vehicle_state(2) < 0.27 || IsUseVehicleFlag) { //车辆速度较小默认为静止状态  起始状态 或者 人为刹车
        ...
        void Pathplanningduringdeceleration( std::vector<Eigen::VectorXd> &GlobalcoordinatesystemObsesLimitinlocal_){
            ...
            if(collision_detection_flag){//若当前减速路径中有障碍物 并且知道当前路径在什么位置发生了碰撞 
                IsUseVehicleFlag = true;
                double distance_threshold = 9;//碰撞位置阈值为10m
                ...
                if(car_collision_detection_distance > distance_threshold &&
                    ...
                    for (size_t i = 0; i < optTrajxy.cols(); ++i){
                        if(optTrajxy(5, i) < optTrajxy(5, closestIndex)){ //车辆当前位置之前的轨迹点 不考虑
                            ...
                            //以恒定减速度 进行减速 在距离障碍物位置distance_threshold远的地方停下来 
    ...
    private:
        ...
        bool IsUseVehicleFlag = false; //是否使用车辆当前自车的信息 不使用局部路径最近点的信息 
        //计算车辆累积静止时间 
        std::chrono::high_resolution_clock::time_point first_time;
        std::chrono::high_resolution_clock::time_point second_time;
        double wait_time; //等待时间的总和 
        bool wait_time_flag = true;

        /*******减速  包含AEB*******/
        bool IsStartDecelerateFlag = false; //启动AEB的标志 
