...
class SpeedVisualizer(Node):
    ...
    def steer_angle_callback(self, msg):
        ...
        # 滤波操作：使用简单的滑动平均滤波
        if len(self.steer_angles) > 5:
            filtered_steer = np.mean(self.steer_angles[-5:])  # 滑动平均
        else:
            filtered_steer = steer_angle  # 如果数据不足，使用原始值
        ...
        self.text_steer.set_text(f'Latest Steer Angle: {steer_angle:.2f}°')
